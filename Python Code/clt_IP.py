...
if int(mat_prompt) == 1:
    ...
    E1_array = np.full((1, number_layers), E1)
    print(E1_array)
    E2 = float(input('E2 [Pa]: '))
    E2_array = np.full((1, number_layers), E2)
    v12 = float(input('v12: '))
    v12_array = np.full((1, number_layers), v12)
    G12 = float(input('G12 [Pa]: '))
    G12_array = np.full((1, number_layers), G12)
else:
...
while count <= number_layers:
    ...
    count = count + 1
Lo_array = np.atleast_2d(Lo_array).T
...
if layer_prompt == 1:
    ...
    thickness_array = np.full((1, number_layers), thickness)
...
df = pd.concat([E1_array, E2_array, v12_array, G12_array, Lo_array, thickness_array], axis=1)
print(df)
# CLT code
# Author: Alex Hernandez-Pryszlak, Daniel Donate-Perez
# Date:   2/15/2022
